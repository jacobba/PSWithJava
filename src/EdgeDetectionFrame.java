...
class EdgeDetectionFrame {
    ...
    private void init(){
        ...
        jb_confirm.addActionListener((ActionEvent e)->{
            switch (jcb_mathods.getSelectedIndex()){
                case 0:
                    new Tools().save_file(new EdgeDetection(input).Sobel(js_threshold.getValue()),jf_edge_detection);
                    break;
                case 1:
                    new Tools().save_file(new EdgeDetection(input).Prewitt(js_threshold.getValue()),jf_edge_detection);
                    break;
                case 2:
                    new Tools().save_file(new EdgeDetection(input).Roberts(),jf_edge_detection);
                    break;
                default:
    ...
    EdgeDetectionFrame(BufferedImage bi){
        input = bi;
        new Tools().ChangeUI();
        init();
